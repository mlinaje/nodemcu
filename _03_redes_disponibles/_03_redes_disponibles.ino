...
void loop() {  
  ...
  if (n == 0)
    ...
    num = Serial.read(); //leer lo que se ha tecleado (un caracter)
    num-=48; //convertir lo leido a numero
    
    if (num!=n){//Si no se ha escogido la opcion de salir

      //Si requiere password preguntar al usuario para que la introduzca
      if (WiFi.encryptionType(num) != ENC_TYPE_NONE){
        Serial.println("intoduce el password: ");
        while (!Serial.available()); //esperar hasta que haya algo tecleado en la consola
        password = Serial.readString(); //ller lo que se teclea por COM (string)
      }
      ...
      while(1)
        delay(100); //aquí iría el código a ejecutar cuando ya sabemos que estamos conectados a la red!
    }  
  }//fin else
}//fin loop
